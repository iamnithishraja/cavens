    ...
    try {
      ...
      const processedMenuItems = await Promise.all(
        menuItems.map(async (item) => {
          ...
          return {
            name: item.name.trim(),
            price: item.price.toString(), // Keep as string to match backend expectation
      ...
      // Prepare event data with correct structure for backend
      const eventData = {
        ...
        tickets: events[0].ticketTypes.map((ticket) => ({
          ...
          type: ticket.name.trim(), // Backend uses 'type' field as well
          price: parseFloat(ticket.price) || 0, // Convert to number for backend
          quantityAvailable: parseInt(ticket.quantity) || 0, // Backend expects 'quantityAvailable'
      ...
      // Send data directly (not wrapped) - matches backend expectation
      const response = await apiClient.post("/api/club/event", eventData);
      ...
      if (response.data.success) {
        Alert.alert(
          ...
          [{ text: "OK", onPress: () => {
            ...
            // You might want to call router.back() or reset state here
          }}]
        );
      } else {
